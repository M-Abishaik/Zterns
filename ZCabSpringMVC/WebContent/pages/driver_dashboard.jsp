...
<body onload="driverdash()" id="driver">
	...
	<div class="vertical-menu">
		<a class="drivernav" onclick="driverdash()">Dashboard</a> <a
			class="drivernav" onclick="driverviewprofile()"
			href="${Config.BASE_PATH}dashboard/driver">Profile</a> <a
			class="drivernav"
			href="${Config.BASE_PATH}dashboard/driver/rides/ongoing">My Trips</a>
	...
	<div class="container3 driver">
		...
		<div id="driverdash">
			...
			<c:if test="${postConfirmInvoice != null}">
				...
				<div class="dashboard-container" style="margin-top: 7%;">
					...
					<br>
					<br>
					<form action="${Config.BASE_PATH}ride/complete" method="post">
						<input type="hidden" name="travelInvoiceBookingID"
							value="${postConfirmInvoice.getBookingID()}" />
						<button class="button button-accent bookride-cancel" type="submit">Complete
							Booking</button>
					</form>
				</div>
		</div>
		...
		<c:if test="${userProfile != null}">
			...
			<form action="${Config.BASE_PATH}users/update" id="driverprofile"
				onsubmit="return validateUpdateProfile()" method="post">
				<input type="hidden" name="profileType" value="1" /> <img
...
<script src="${Config.BASE_PATH}/js/dashdriver.js"></script>
<script
